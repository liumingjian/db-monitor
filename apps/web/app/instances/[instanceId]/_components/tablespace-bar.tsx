...
export function TablespaceBar({ percent, band }: TablespaceBarProps) {
	...
	return (
		<div aria-label={`使用率 ${formatPercent(percent)}`} className="flex items-center gap-2">
			<div className="h-2 w-28 overflow-hidden rounded-full bg-black/10">
				<div className={`h-full ${fillClass}`} data-band={band} style={{ width: `${clamped}%` }} />
			</div>
			<span className="tabular-nums text-xs text-[var(--muted)]">{formatPercent(percent)}</span>
		</div>
	);
}
